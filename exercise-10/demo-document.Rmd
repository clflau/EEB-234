...
Given that we have  those two entries in the `biblio.bib` file, we can cite them in the document using the syntax `Here's a sentence that cites both papers [@Cooney_2017; @Silvestro_2014]`. Here's a sentence that cites both papers [@Cooney_2017; @Silvestro_2014; @Rabosky_2013].  
...
## Showing python code in rmarkdown
```{python, eval=F}
f = open("/path/to/file")
lines = f.readlines()
```
